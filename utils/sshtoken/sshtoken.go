			...
			if len(agentSigners) > 0 {
				...
				if publicKeyOverride == nil {
					...
					for _, identityFile := range publicKeyIdentities {
						...
						keybytes, _ := os.ReadFile(identityFile)
						pubkey, _, _, _, _ := ssh.ParseAuthorizedKey(keybytes)
						identities[identityFile] = pubkey
					}
				} else {
					// append to identityfiles
					keybytes, _ := os.ReadFile(*publicKeyOverride)
					pubkey, _, _, _, _ := ssh.ParseAuthorizedKey(keybytes)
...
func parsePublicKeyFile(path string) (string, error) {
	...
	return fmt.Sprintf("%s %s", pubkeySplit[0], pubkeySplit[1]), nil
}
